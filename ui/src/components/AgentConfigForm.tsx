...
export function AgentConfigForm(props: AgentConfigFormProps) {
  ...
  const effectiveAdapterConfig = useMemo(() => {
    if (props.mode === "create") {
      const adapter = getUIAdapter(props.values.adapterType);
      return adapter.buildAdapterConfig(props.values);
    }
    return props.agent.adapterConfig;
  }, [props]);
  ...
  const testEnvironment = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["agents", selectedCompanyId, "adapter-models", adapterType],
      });
    },
  });
