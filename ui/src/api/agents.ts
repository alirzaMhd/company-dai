...
export const agentsApi = {
  ...
  testEnvironment: (
    ...
    api.post<AdapterEnvironmentTestResult>(
      `/api/adapters/${type}/test-environment`,
      data,
    ),
  ...
  createAgentApiKey: (companyId: string, data: { name: string; key: string; provider?: string }) =>
    api.post<CompanyAgentApiKey>(`/companies/${companyId}/agent-api-keys`, data),
  listAgentApiKeys: (companyId: string, provider?: string) =>
    api.get<CompanyAgentApiKey[]>(
      `/companies/${companyId}/agent-api-keys${provider ? `?provider=${encodeURIComponent(provider)}` : ""}`,
    ),
  deleteAgentApiKey: (companyId: string, keyId: string) =>
    api.delete<{ ok: boolean }>(`/companies/${companyId}/agent-api-keys/${keyId}`),
};
...
export interface CompanyAgentApiKey {
  id: string;
  name: string;
  provider: string;
