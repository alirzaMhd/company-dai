...
import { buildPersistentSkillSnapshot, ensurePaperclipSkillSymlink, readPaperclipRuntimeSkillEntries, readInstalledSkillTargets, resolvePaperclipDesiredSkillNames, } from "../../adapter-utils/dist/server-utils.js";
function asString(value) {
    return typeof value === "string" && value.trim().length > 0 ? value.trim() : null;
}
function resolveOpenCodeRemoteSkillsHome(config) {
...
async function buildOpenCodeRemoteSkillSnapshot(config) {
...
export async function listOpenCodeRemoteSkills(ctx) {
    return buildOpenCodeRemoteSkillSnapshot(ctx.config);
}
export async function syncOpenCodeRemoteSkills(ctx, desiredSkills) {
...
export function resolveOpenCodeRemoteDesiredSkillNames(config, availableEntries) {
