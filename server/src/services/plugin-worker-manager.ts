...
export function createPluginWorkerHandle(
  ...
  function attachStdioHandlers(child: ChildProcess): void {
    ...
    if (child.stdout) {
      readline = createInterface({ input: child.stdout, crlfDelay: 0 });
      readline.on("line", handleLine);
    }

    // Capture stderr for logging
    if (child.stderr) {
      stderrReadline = createInterface({ input: child.stderr, crlfDelay: 0 });
  ...
  async function startInternal(): Promise<void> {
    ...
    log.info({ pid: child.pid, entrypoint: options.entrypointPath }, "worker process spawned");
    ...
    try {
      log.info("worker: sending initialize RPC call");
      ...
      log.info({ supportedMethods }, "worker initialize succeeded");
    } catch (err) {
