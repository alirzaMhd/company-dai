      ...
      if (db) {
        ...
        try {
          readyPlugins = await Promise.race([
            pluginRegistry.listByStatus("ready") as Promise<PluginRecord[]>,
            new Promise<never>((_, reject) =>
              setTimeout(() => reject(new Error("Plugin query timeout in tool dispatcher")), QUERY_TIMEOUT_MS)
            ),
          ]);
        } catch (err) {
          console.error("[DEBUG] plugin-tool-dispatcher: query failed:", err);
          readyPlugins = [];
        }
