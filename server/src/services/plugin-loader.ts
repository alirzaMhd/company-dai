...
export function pluginLoader(
  ...
  return {
    ...
    async loadAll(): Promise<PluginLoadAllResult> {
      ...
      let readyPlugins: PluginRecord[] = [];
      const QUERY_TIMEOUT_MS = 8000;

      try {
        readyPlugins = await Promise.race([
          registry.listByStatus("ready") as Promise<PluginRecord[]>,
          new Promise<never>((_, reject) =>
            setTimeout(() => reject(new Error("Plugin query timeout")), QUERY_TIMEOUT_MS)
          ),
        ]);
      } catch (err) {
        console.error("[DEBUG] plugin-loader: query failed:", err);
        readyPlugins = [];
      }

      log.info({ count: readyPlugins.length }, "plugin-loader: ready plugins fetched");
      ...
      const loadTimeoutHandle = setTimeout(() => {
        ...
        log.warn(
          { elapsed: LOAD_ALL_TIMEOUT_MS },
          "plugin-loader: loadAll timeout exceeded, continuing with partial results",
        );
      }, LOAD_ALL_TIMEOUT_MS);

      const loadResults: PluginLoadResult[] = [];
      await Promise.all(
        readyPlugins.map(async (plugin, i) => {
          if (pluginLoadTimedOut) {
            loadResults[i] = {
              plugin,
              success: false,
              error: "Load timeout exceeded",
              registered: { worker: false, eventSubscriptions: 0, jobs: 0, webhooks: 0, tools: 0 },
            };
            return;
          }
          try {
            loadResults[i] = await activatePlugin(plugin);
          } catch (err) {
            loadResults[i] = {
              plugin,
              success: false,
